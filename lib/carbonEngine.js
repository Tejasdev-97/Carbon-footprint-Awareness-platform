...
/**
 * Default one-way commute distance used when a log entry has no `km` field.
 * 10 km matches the average Indian urban one-way office commute (TERI 2023).
 * @constant {number}
 */
const DEFAULT_COMMUTE_KM = 10

/**
 * Multiplier used to convert CO₂ savings (kg) into estimated money saved (₹).
 * Approximates the weighted cost of petrol + public transport in India.
 * @constant {number}
 */
const INR_PER_KG_CO2_COMMUTE = 150 // ~₹150 per kg CO₂ for vehicle fuel equivalent
/** @constant {number} */
const INR_PER_KG_CO2_COOKING = 80   // ~₹80 per kg CO₂ for LPG equivalent
/** @constant {number} */
const INR_PER_KG_CO2_FOOD = 60      // ~₹60 per kg CO₂ for protein food cost
/** @constant {number} */
const INR_PER_KG_CO2_ENERGY = 120   // ~₹120 per kg CO₂ for electricity units
/** @constant {number} */
const INR_PER_KG_CO2_WASTE = 40     // ~₹40 per kg CO₂ for single-use packaging

/**
 * Score factor: each kg of daily CO₂ deducts this many points from 100.
 * Calibrated so that 12.5 kg CO₂/day (high Indian household) scores 0.
 * @constant {number}
 */
const CO2_SCORE_FACTOR = 8
...
function entryToCO2(entry) {
  ...
  switch (category) {
    case 'commute': {
      ...
      const km = Number(entry.km) || DEFAULT_COMMUTE_KM
      return factor * km
    }
    case 'energy': {
...
export function calculateDailyScore(logEntries = []) {
  ...
  // score = clamp(100 - (totalCO2 * CO2_SCORE_FACTOR), 0, 100)
  const score = clamp(Math.round(100 - totalCO2 * CO2_SCORE_FACTOR), 0, 100)
...
export function getSwapSuggestion(breakdown = {}) {
  ...
  if (commute > 0.5) {
    suggestions.push({
      ...
      impactMoneySaved: Math.round(commute * INR_PER_KG_CO2_COMMUTE),
  ...
  if (cooking > 0.8) {
    suggestions.push({
      ...
      impactMoneySaved: Math.round(cooking * INR_PER_KG_CO2_COOKING),
  ...
  if (food > 1.0) {
    suggestions.push({
      ...
      impactMoneySaved: Math.round(food * INR_PER_KG_CO2_FOOD),
  ...
  if (energy > 1.0) {
    suggestions.push({
      ...
      impactMoneySaved: Math.round(energy * INR_PER_KG_CO2_ENERGY),
  ...
  if (waste > 0.1) {
    suggestions.push({
      ...
      impactMoneySaved: Math.round(waste * INR_PER_KG_CO2_WASTE),
