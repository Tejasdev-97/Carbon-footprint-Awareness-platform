...
/**
 * Multiplier that converts total daily CO₂ (kg) into a deduction from 100.
 * Derived from: the average Indian household produces ~12.5 kg CO₂/day;
 * at that level the score reaches 0. 100 / 12.5 ≈ 8.
 * @constant {number}
 */
...
export function analyzeDay(text) {
  ...
  // Calculate score: clamp(100 - totalCO2 * CO2_SCORE_FACTOR, 0, 100)
  // Matches the formula in carbonEngine.js for consistency across modules.
  const score = Math.max(0, Math.min(100, Math.round(100 - totalCO2 * CO2_SCORE_FACTOR)))
