...
/**
 * Returns a smart fallback response based on the user prompt and language.
 * @param {string} prompt
 * @param {string} language - BCP-47 language code e.g. 'hi-IN'
 * @returns {string}
 */
function getFallbackResponse(prompt, language) {
  const query = (prompt || '').toLowerCase()
  
  // Localized smart fallback responses
  const smartFallbacks = {
    'en-IN': {
      carbon: "Carbon footprint refers to the total amount of greenhouse gases, primarily carbon dioxide, emitted by human activities like driving, electricity use, and food consumption.",
      today: "Your carbon footprint today is calculated based on your activities. Try logging your commute, food, and energy in the Track tab to see your real-time score!",
      footprint: "Your carbon footprint represents your environmental impact. Lowering it helps fight climate change. Try walking, using public transit, and reducing single-use plastics.",
      city: "Your city score is affected by all citizens' pledges and actions. Log your activities and complete daily challenges to help improve your city's health!",
      help: "I can help you understand your carbon footprint, explain sustainability concepts, or suggest ways to improve your daily score. What would you like to know?",
    },
    'hi-IN': {
      carbon: "कार्बन फुटप्रिंट का मतलब उन ग्रीनहाउस गैसों (विशेषकर कार्बन डाइऑक्साइड) की कुल मात्रा से है, जो हमारे दैनिक कार्यों जैसे वाहन चलाने और बिजली के उपयोग से उत्पन्न होती हैं।",
      today: "आज का आपका कार्बन पदचिह्न आपकी गतिविधियों पर आधारित है। अपना स्कोर देखने के लिए ट्रैक टैब में अपना आवागमन, भोजन और ऊर्जा लॉग करें!",
      footprint: "आपका कार्बन पदचिह्न पर्यावरण पर आपके प्रभाव को दर्शाता है। इसे कम करने से जलवायु परिवर्तन से लड़ने में मदद मिलती है। पब्लिक ट्रांसपोर्ट का उपयोग करें।",
      city: "आपके शहर का स्कोर सभी नागरिकों की प्रतिज्ञाओं और कार्यों से प्रभावित होता है। अपने शहर के स्वास्थ्य को बेहतर बनाने में मदद के लिए गतिविधियों को लॉग करें।",
      help: "मैं आपको कार्बन फुटप्रिंट समझने, स्थिरता के बारे में बताने या अपने दैनिक स्कोर को सुधारने के तरीके सुझाने में मदद कर सकता हूँ।",
    }
  }

  const langKey = smartFallbacks[language] ? language : 'en-IN'
  const dict = smartFallbacks[langKey]

  if (query.includes('carbon')) return dict.carbon
  if (query.includes('today') || query.includes('footprint')) return dict.today || dict.footprint
  if (query.includes('city') || query.includes('score')) return dict.city
  if (query.includes('help') || (query.includes('what') && query.includes('you'))) return dict.help

  // Fallback to random generic tip
...
export async function getAIInsight(prompt, language = 'en-IN') {
  ...
  try {
    ...
    return { text: getFallbackResponse(sanitized, language), source: 'fallback' }
  }
...
Context: The user is tracking their daily carbon footprint through the Prithvi app.`
  ...
  try {
    ...
    if (
      ...
      return {
        text: getFallbackResponse(sanitized, language),
        source: 'fallback',
      }
    }
